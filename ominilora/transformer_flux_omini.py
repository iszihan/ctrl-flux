...
class FluxOminiAttnProcessor(FluxAttnProcessor):
    ...
    def __call__(self, 
        ...
        queries, keys, values = [], [], []
        for i, hidden_state in enumerate(hidden_states):
            with specify_lora((attn.to_q, attn.to_k, attn.to_v), adapters[i+adapter_offset]):
                query, key, value, encoder_query, encoder_key, encoder_value = _get_qkv_projections(
                    attn, hidden_state, encoder_hidden_states if encoder_hidden_states is not None and i == 0 else None
                    )
                ...
                if i ==0 and encoder_hidden_states is not None:
                    ...
                    encoder_query, encoder_key = attn.norm_added_q(encoder_query), attn.norm_added_k(encoder_key)
        ...
        for i, _ in enumerate(hidden_states):
            ...
            if getattr(attn, "to_out", None) is not None:
                with specify_lora((attn.to_out[0],), adapters[i+adapter_offset]):
                    hidden_states[i] = attn.to_out[0](hidden_states[i])
            hidden_states_out.append(hidden_states[i])
        if encoder_hidden_states is not None:
            encoder_hidden_state_out = attn.to_add_out(attention_outputs[0])
            return hidden_states_out, encoder_hidden_state_out
        else:
            return hidden_states_out

class FluxOminiAttention(FluxAttention):
    _default_processor_cls = FluxOminiAttnProcessor
    _available_processors = [
        FluxOminiAttnProcessor,
    ]
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Re-set attention processor to Omini-specific processor
        processor = kwargs.get("processor", self._default_processor_cls())
    ...
    def forward(self, 
        ...
        return self.processor(self, hidden_states, encoder_hidden_states, position_embs, group_mask, adapters, **kwargs)

class FluxOminiTransformerBlock(FluxTransformerBlock):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Same params as parent; we're called with dim=..., num_attention_heads=..., attention_head_dim=...
        dim = kwargs["dim"]
        num_attention_heads = kwargs["num_attention_heads"]
        attention_head_dim = kwargs["attention_head_dim"]
        eps = kwargs.get("eps", 1e-6)
        self.attn = FluxOminiAttention(
            query_dim=dim,
            ...
            bias=True,
            processor=FluxOminiAttnProcessor(),
            eps=eps,
        )
...
class FluxOminiSingleTransformerBlock(FluxSingleTransformerBlock):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        dim = kwargs["dim"]
        num_attention_heads = kwargs["num_attention_heads"]
...
class FluxOminiTransformer2DModel(FluxTransformer2DModel):
    def __init__(
        self,
        patch_size: int = 1,
        in_channels: int = 64,
        ...
        # Convert existing blocks to Omini versions in-place (no re-allocation).
        # Only swaps __class__ (for the Omini forward) and attention processor.
        for block in self.transformer_blocks:
            block.__class__ = FluxOminiTransformerBlock
            block.attn.__class__ = FluxOminiAttention
            block.attn.set_processor(FluxOminiAttnProcessor())

        for block in self.single_transformer_blocks:
            block.__class__ = FluxOminiSingleTransformerBlock
            block.attn.__class__ = FluxOminiAttention
            block.attn.set_processor(FluxOminiAttnProcessor())
    ...
    def forward(self, 
        ...
        for block in self.transformer_blocks:
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                hidden_states_all, encoder_hidden_states = self._gradient_checkpointing_func(
                    block,
                    hidden_states_all,
                ...
                hidden_states_all, encoder_hidden_states = block(
                    hidden_states=hidden_states_all,
                    encoder_hidden_states=encoder_hidden_states,
                    tembs=tembs,
                    position_embs=position_embs,
        ...
        for block in self.single_transformer_blocks:
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                hidden_states_all = self._gradient_checkpointing_func(
                    block,
                    hidden_states_all,
                    tembs,
                    position_embs,
